...
import com.example.algorithm.kotlin_coroutine.data.Article
import com.example.algorithm.kotlin_coroutine.data.ArticleRepository
import com.example.algorithm.kotlin_coroutine.data.User01
import com.example.algorithm.kotlin_coroutine.data.Users
import com.example.algorithm.kotlin_coroutine.data.UserAction
import com.example.algorithm.kotlin_coroutine.data.UserDataRepository
import com.example.algorithm.kotlin_coroutine.data.UserDataView
import com.example.algorithm.kotlin_coroutine.data.UserProfileData
import com.example.algorithm.kotlin_coroutine.data.getNotifications
import com.example.algorithm.kotlin_coroutine.data.getUserData
import com.example.algorithm.kotlin_coroutine.data.repo
...
suspend fun produceCurrentUserSeq(): User01 {
    val profile = repo.getProfile()
    val friends = repo.getFriends()
    return User01(profile, friends)
}
suspend fun produceCurrentUserSym(): User01 = coroutineScope {
    val profile = async { repo.getProfile() }
    val friends = async { repo.getFriends() }
    User01(profile.await(), friends.await())
}



// withContext(EmptyCoroutineContext)와 CoroutineScope은 완벽히 같은 방식으로 동작!
fun CoroutineScope.log1(text: String) {
    val name = coroutineContext[CoroutineName]?.name
    println("[$name] $text")
}
...
// After
    ...
    suspend fun notifyAnalyticsAction(action: UserAction) {
// ...
    }

    // supervisorScope를 사용해, 개별액션처리중에 발생하는 예외가 다른 액션처리에 영향을 주지 않도록하는 코드
// 병렬처리가 필요하면서도, 서로 독립적인 작업을 시작해야하는 상황에서 주로 사용
    suspend fun notifyAnalytics(actions: List<UserAction>) =
        supervisorScope {
            actions.forEach { action ->
                launch {
                    notifyAnalyticsAction(action)
                }
            }
        }
...
// 이를통해 끝날때까지 기다리지 않고, 만약 notifyProfileShown()에서 예외가 발생하더라도 얘때문에 다른자식이 취소되지X
    class ShowserDataUseCase(
        ...
        suspend fun showUserData() = coroutineScope {
            ...
            view.show(user)
            val analyticsScope =
                CoroutineScope(SupervisorJob())    //생성자를 통해 SupervisorJob()을 주입해서, 추가적인 연산을 위한 스코프를 만듦 => 끝날때까지 기다리지X
